$(document).ready(function(){ // pour commencer à manipuler des eléments de mon dom html une fois chargé
    ...
    $('.supprlivre').click(function(e){
    console.log(e); // lire mon event
    console.log(e.originalEvent.path[7].id); // aller chercher la donnée qui m'interesse ici l'id du madalvue qui est egal à celui des livres. 
    ...
    $.ajax({
        type: "POST",
        url: "http://localhost/Biblial3/supprimerlivre.php",
        data:{
            livre: idlivre, // data qui correspond à l'id du livre
            userconnecté : user, // data qui correspond à l'id de l'user co 
        },
        success: function(data){
            $('.supprlivre').popover('show');
           /*  window.location.replace("http://localhost/Biblial3/macollection.php"); */
